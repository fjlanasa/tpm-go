...
func NewGraph(ctx context.Context, cfg config.GraphConfig, outlet *chan any) (*Graph, error) {
	...
	graph := &Graph{pipelines: []Pipeline{}, outlet: outlet}
	for _, pipelineConfig := range cfg.Pipelines {
		if outlet != nil {
			pipelineConfig.Sinks = append(pipelineConfig.Sinks, config.SinkConfig{
				ID:   config.ID("outlet"),
