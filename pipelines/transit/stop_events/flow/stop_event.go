...
type StopEventFlow struct {
	in            chan any
	out           chan any
	vehiclesState state_stores.StateStore[*pb.VehiclePositionEvent]
}

func NewStopEventFlow(ctx context.Context, state ...state_stores.StateStore[*pb.VehiclePositionEvent]) *StopEventFlow {
	var vehiclesState state_stores.StateStore[*pb.VehiclePositionEvent]
	if len(state) > 0 {
