...
func NewDwellEventFlow(ctx context.Context, stateStore ...state_stores.StateStore[*pb.StopEvent]) *DwellEventFlow {
	if len(stateStore) == 0 {
		stateStore = []state_stores.StateStore[*pb.StopEvent]{
...
func (d *DwellEventFlow) process(event *pb.StopEvent) {
	...
	if found && event.GetEventType() == pb.StopEvent_DEPARTURE {
		...
		dwellSeconds := int32(event.GetStopTimestamp().AsTime().Sub(currentState.GetStopTimestamp().AsTime()).Seconds())
