...
type TravelTimeEventFlow struct {
	in         chan any
	out        chan any
	tripStates state_stores.StateStore[*pb.StopEvent]
}

func NewTravelTimeEventFlow(ctx context.Context, stateStore ...state_stores.StateStore[*pb.StopEvent]) *TravelTimeEventFlow {
	if len(stateStore) == 0 {
